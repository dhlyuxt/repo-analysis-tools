...
| --- | --- | --- | --- |
| `codewiki/scan/service.py` and `codewiki/scan/models.py` | migrate and reorganize | Approved inventory marks the scan core as deterministic offline logic, while storage-backed scan persistence is classified separately below. | Move runtime assets from `.claude/codewiki/` to `.codewiki/`; keep scan IDs, provenance capture, and downstream bindings. |
...
| `codewiki/anchors/service.py` and `codewiki/anchors/adapter.py` | migrate and reorganize | Anchor extraction is deterministic and central to later slice and impact flows, while anchor persistence is classified separately below. | Keep structural extraction logic; storage ownership for `codewiki/anchors/storage.py` lives with the storage row, not here. |
...
| `codewiki/evidence/builder.py`, `codewiki/evidence/citations.py`, and `codewiki/evidence/models.py` | migrate and reorganize | Context-pack construction, citations, and evidence-pack structure are durable core logic. | Keep typed evidence assets and citation rules; shell rendering concerns are classified elsewhere. |
...
| `codewiki/storage/repo_store.py`, `codewiki/storage/slice_store.py`, `codewiki/storage/migrations.py`, plus related `codewiki/scan/storage.py` and anchor persistence in `codewiki/anchors/storage.py` | migrate and reorganize | Storage is durable cross-cutting infrastructure for scan state, repo metadata, slice persistence, and anchor persistence. Why this keep is safe: the durable part is the asset store and latest-state model, not the runtime telemetry shell. | Rebuild schema/layout under `.codewiki/`; keep scan state, repo asset persistence, slice persistence, and anchor persistence hooks now; do not assume old storage-backed report/export handles from this row. |
| `codewiki/storage/session_store.py` and `codewiki/storage/trace_store.py` | drop | Capability mapping explicitly says session/trace persistence is not automatically mapped forward. | Exclude runtime session telemetry, session replay logs, and trace-event history from M1 by default. |
...
| `codewiki/evidence/renderers.py` | extract logic, rewrite shell | Citation-aware render helpers belong to the legacy report shell rather than the durable evidence core. | Harvest any useful rendering helpers into the rewritten report path; do not classify this file as kept evidence-domain logic. |
...
| `builds/easyflash-e2e-demo/` | test-only baseline | Inventory identifies this tree as reference-only comparison material rather than an accepted M1 fixture baseline. | Keep only as reference/comparison material; do not treat it as an accepted real-fixture gate for M1. |
...
| `codewiki/ask/*` and `codewiki/answers/*` | extract logic, rewrite shell | The useful part is the decomposition into slice, evidence, impact, and conservative answer structure; the top-level ask product surface is explicitly non-mapped. | Keep answer-contract ideas where they inform typed result shapes and impact reasoning, but do not preserve `ask` as a mega-surface. |
| `codewiki/cli/*` plus related `codewiki/demo/*` and `codewiki/benchmarks/*` shells | drop | CLI compatibility commands, bundle/demo export shells, and benchmark harnesses do not map forward as product surfaces. | Keep only any golden expectations indirectly through fixtures/tests; no CLI compatibility or demo-first shell survives. This drop row does not override the explicit export harvest exceptions above for `codewiki/cli/bundle.py::export_bundle` and `codewiki/demo/export.py::run_scope_first_demo_export`. |
