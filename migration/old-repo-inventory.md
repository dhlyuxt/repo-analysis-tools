...
- Tests already encode the intended scope-first legacy golden path: build repo fixture, run scan, answer a locate-symbol question, compute impact, and render the legacy report shell; export behavior is tracked separately.
...
| reporting shell | `codewiki/reporting/service.py`, `codewiki/evidence/renderers.py` | `ReportingService.render_focus_report` | `tests/test_reporting/test_service.py`, `tests/test_cli/test_main.py`, `tests/test_pipeline/test_scope_first_pipeline.py` | Mostly yes; writes markdown output | `report` | extract logic, rewrite shell | Renders focus reports from context packs and writes them under `docs/codewiki/`. |
| legacy export bridge | `codewiki/reporting/service.py` | `ReportingService.export` | `tests/test_reporting/test_service.py`, `tests/test_cli/test_main.py`, `tests/test_pipeline/test_bundle_export.py`, `tests/test_pipeline/test_demo_export.py` | Mostly yes; writes export output | `export` | extract logic, rewrite shell | Keeps the legacy export-oriented packaging path separate from report generation so M1 can harvest it under the new export domain. |
