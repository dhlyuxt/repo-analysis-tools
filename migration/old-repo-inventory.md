...
- Old-repo sources inspected for this baseline:
  ...
  - `codewiki/cli/bundle.py`
  - `codewiki/demo/export.py`
  ...
  - `tests/test_agent_runtime/test_expand_slice_tool.py`
  - `tests/test_agent_runtime/test_orchestrator.py`
  - `tests/test_agent_runtime/test_telemetry.py`
  - `tests/test_anchors/test_service.py`
  - `tests/test_anchors/test_storage.py`
  ...
  - `tests/test_retrieval/test_freshness.py`
  - `tests/test_pipeline/test_scope_first_pipeline.py`
  - `tests/test_pipeline/test_bundle_export.py`
  - `tests/test_pipeline/test_demo_export.py`
  - `tests/test_reporting/test_service.py`
  - `tests/test_retrieval/test_service.py`
  - `tests/test_scan/test_service.py`
  - `tests/test_scan/test_storage.py`
  - `tests/test_scope/test_service.py`
  - `tests/test_slices/test_planner.py`
  - `tests/test_slices/test_expansion_recipes.py`
  - `tests/test_storage/test_repo_store.py`
  - `tests/test_storage/test_session_store.py`
  - `tests/test_storage/test_slice_store.py`
  - `tests/test_storage/test_trace_store.py`
...
| --- | --- | --- | --- | --- | --- |
| Scope-first mainline | `scan`, `scope`, `anchors`, `slices`, `evidence`, `impact` | README workflow plus service modules | `scan`, `scope`, `anchors`, `slice`, `evidence`, `impact` | migrate and reorganize | Deterministic offline core worth harvesting. |
| Reporting shell | `reporting/service.py`, `evidence/renderers.py` | Scope-first focus-report flow | `report` | extract logic, rewrite shell | Keep structured report logic, but rewrite the old markdown/file-output shell. |
...
- Tests already encode the intended scope-first legacy golden path: build repo fixture, run scan, answer a locate-symbol question, compute impact, and render the legacy report shell; bundle/demo export behavior is tracked separately as synthetic format checks on `build_scope_first_repo`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| scan core | `codewiki/scan/service.py`, `codewiki/scan/models.py` | `ScanService.scan`, `ScanPaths.for_repo` | `tests/test_scan/test_service.py`, `tests/test_scan/test_storage.py`, `tests/test_pipeline/test_scope_first_pipeline.py` | Yes; filesystem + git provenance only | `scan` | migrate and reorganize | Builds repo-local scan state and triggers scope and anchor extraction; storage ownership for `codewiki/scan/storage.py` lives with the storage row below. |
...
| reporting shell | `codewiki/reporting/service.py`, `codewiki/evidence/renderers.py` | `ReportingService.render_focus_report`, `ReportingService.export` | `tests/test_reporting/test_service.py`, `tests/test_cli/test_main.py`, `tests/test_pipeline/test_scope_first_pipeline.py` | Mostly yes; writes markdown output | `report` | extract logic, rewrite shell | Renders focus reports from context packs and writes them under `docs/codewiki/`; `ReportingService.export` is the legacy report-shell writer path, not the export domain seed. |
| legacy export surfaces | `codewiki/cli/bundle.py`, `codewiki/demo/export.py` | `export_bundle`, `run_scope_first_demo_export` | `tests/test_pipeline/test_bundle_export.py`, `tests/test_pipeline/test_demo_export.py` | Mostly yes; writes export output | `export` | extract logic, rewrite shell | Keeps the bundle/demo export-oriented packaging paths separate from report generation so M1 can harvest the actual export surfaces under the new export domain. |
| core storage layer | `codewiki/storage/repo_store.py`, `codewiki/storage/slice_store.py`, `codewiki/storage/migrations.py`, `codewiki/scan/storage.py`, `codewiki/anchors/storage.py` | `RepoStore.for_repo`, `SliceStore.record_slice_run`, `AnchorStorage.replace`, `ScanStorage.record_scan` | `tests/test_storage/test_repo_store.py`, `tests/test_storage/test_slice_store.py`, `tests/test_anchors/test_storage.py`, `tests/test_scan/test_storage.py` | Yes | cross-cutting storage support layer | migrate and reorganize | Owns SQLite schema, latest views, and durable scan/slice/repo/anchor persistence used by the analysis domains; it does not imply old report/export handle infrastructure. |
| runtime storage shells | `codewiki/storage/session_store.py`, `codewiki/storage/trace_store.py` | `SessionStore.record_tool_event`, `TraceStore.record_span` | `tests/test_storage/test_session_store.py`, `tests/test_storage/test_trace_store.py`, `tests/test_agent_runtime/test_telemetry.py` | Yes, but runtime-specific | none | drop | Persists runtime session telemetry and trace linkage that do not carry forward by default. |
...
| real fixture baselines | `EasyFlash-master`, `builds/easyflash-e2e-clean`, `builds/easyflash-e2e-demo` | `AGENTS.md:29`, `testscript/test_partition.py:22`, `builds/easyflash-e2e-artifacts/manifest.json:3` | no direct pipeline tests; decision grounded in repo references and checked-in artifacts | N/A as inventory item | fixtures | test-only baseline | `EasyFlash-master` is the accepted primary real-fixture baseline for M1; `builds/easyflash-e2e-clean` is an optional secondary regression snapshot; `builds/easyflash-e2e-demo` is reference-only comparison material and is not part of the accepted M1 fixture baseline. The bundle/demo tests elsewhere are synthetic format checks on `build_scope_first_repo`, not direct real-fixture validation. |
...
- Synthetic baseline fixture:
  ...
  - `tests/test_pipeline/test_scope_first_pipeline.py` proves the current legacy golden path: `scan -> ask -> impact -> report shell`, with export behavior tracked separately.
  - `tests/test_pipeline/test_bundle_export.py` and `tests/test_pipeline/test_demo_export.py` validate bundle/demo export formatting against `build_scope_first_repo`, not against the EasyFlash fixtures.
...
- Real fixture baselines:
  ...
  - `builds/easyflash-e2e-demo`
    Evidence: `builds/easyflash-e2e-demo/README.md` is a concrete checked-in fixture tree with embedded demo sources under `demo/` and library sources under `easyflash/`, but it is reference-only and not part of the accepted M1 fixture baseline.
  - Task 4 accepted handoff: `EasyFlash-master` is the unambiguous primary real-fixture baseline for M1, while `builds/easyflash-e2e-clean` remains an optional secondary regression snapshot and `builds/easyflash-e2e-demo` remains reference-only comparison material.
